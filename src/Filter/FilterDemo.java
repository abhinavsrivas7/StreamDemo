...
static Scanner s = new Scanner(System.in);
	...
	public static void main(String[] args) {
		...
		switch(choice) {
			...
			int[] incrementedNumbers = Arrays.stream(nums).filter(n -> n > 3).toArray();
			printNumbers(incrementedNumbers);
			break;
			
			case 2: List<Person> persons = inputPersons();
			List<Person> changedPersons = persons.stream().filter(p -> {
	...
	private static void printPersons(List<Person> persons) {
		System.out.println("entered persons after filters are: ");
	...
	private static void printNumbers(int[] nums) {
		System.out.println("entered numbers after filters are: ");
